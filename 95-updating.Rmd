...
We've found that updating goes most smoothly when you start with a fresh R session. So if you already have RStudio open, press Ctrl+Shift+F10 (or use the menu and navigate to Session | Restart R). This will restart R without restarting RStudio. 

In order to manage the updating process smoothly, we'll use another package called `devtools`. We use `devtools` to install the Disco Engine in the first place, so you likely already have it installed. But it doesn't hurt to check for updates:

```{r, eval = FALSE}
update.packages("devtools")
```

If `devtools` is already up-to-date, nothing will happen and you'll be immediately prompted with the `>` prompt. Otherwise, you'll see some messages about the installation process, ending with something like 

```
package 'devtools' successfully unpacked and MD5 sums checked
```

Now we're ready to update the Disco Engine. To do that, just type:
...
That should do it! The `devtools` package that's managing the update is smart enough to make any other necessary updates to other packages that `discoveryengine` depends on, so typing that one line might result in updates to several packages. 
