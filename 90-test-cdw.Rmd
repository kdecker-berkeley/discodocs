...
# we also update the current R session about the new set up
# (this is so we don't have to restart for the changes to take effect)
do.call(Sys.setenv, list(R_LIBS_USER = package_dir))

# finally we update where R will look when loading a package
.libPaths(package_dir)
```

## Installing getcdw

Before you can install getcdw, you'll need to install a few packages. Once again, just copy and paste the following into the console:
...
Now go ahead and use devtools to install getcdw. You'll need to run this line of code:
